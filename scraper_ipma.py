...
def capturar_mapas_PIR():
    ...
    # Variável para memorizar o mapa de Amanhã e usar no dia de Depois de Amanhã
    dados_memoria_amanha = None
    ...
    for d in range(3):
        ...
        # O IPMA SÓ fornece d=0 e d=1 no JSON.
        if d < 2:
            ...
            if resposta.status_code == 200:
                dados = resposta.json()
                if d == 1:
                    dados_memoria_amanha = dados # Guarda na memória para o d=2
            else:
                dados = {}
        else:
            # Para d=2, usamos a cópia dos dados de d=1
            print("   [Aviso] IPMA não fornece Dia 3 (d=2). A replicar projeção do Dia 2 para estabilidade.")
            dados = dados_memoria_amanha if dados_memoria_amanha else {}
            
        # AGORA SIM: A leitura correta da árvore do IPMA (gaveta 'local' -> 'data' -> 'rcm')
        lista_registos = dados.get('local', {})
        x_pos, y_pos = 10, 950
        
        for id_concelho, info in lista_registos.items():
            if isinstance(info, dict):
                dados_concelho = info.get('data', {})
        ...
        plt.close()
        print(f"   Sucesso: {nome_ficheiro} desenhado e guardado.")
